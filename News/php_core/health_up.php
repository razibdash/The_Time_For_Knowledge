<?php 
    ...
    if(isset($_REQUEST['submit']) && isset($_REQUEST['newsHealth']) )
    {
        ...
        if(!empty($imageName))
        {
            ...
            move_uploaded_file($image_tmp_name,$loc.$imageName);
              ...
              $data_insert="INSERT INTO `health`(`news_title`, `picture`, `details`)
               VALUES ('$title_name','$imageName','$news_details')";
              $data = mysqli_query($connect,$data_insert);
              if($data)
              {
                echo '<script>window.location.href="../health_update.php"</script>';
              }else
              {
                echo '<script>window.location.href="../health_update.php?addedError=true"</script>';
              }
