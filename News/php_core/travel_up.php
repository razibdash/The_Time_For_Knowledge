<?php 
    ...
    if(isset($_REQUEST['submit']) && isset($_REQUEST['newsTravel']) )
    {
        ...
        if(!empty($imageName))
        {
            ...
            move_uploaded_file($image_tmp_name,$loc.$imageName);
              ...
              $data_insert="INSERT INTO `travel`(`news_title`, `picture`, `details`)
               VALUES ('$title_name','$imageName','$news_details')";
              $data = mysqli_query($connect,$data_insert);
              if($data)
              {
                echo '<script>window.location.href="../travel_update.php"</script>';
              }else
              {
                echo '<script>window.location.href="../travel_update.php?addedError=true"</script>';
              }
