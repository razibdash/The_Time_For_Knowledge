<?php 
    ...
    if(isset($_REQUEST['submit']) && isset($_REQUEST['newsTechnology']) )
    {
        ...
        if(!empty($imageName))
        {
            ...
            move_uploaded_file($image_tmp_name,$loc.$imageName);
              ...
              $data_insert="INSERT INTO `technology`(`news_title`, `picture`, `details`)
               VALUES ('$title_name','$imageName','$news_details')";
              $data = mysqli_query($connect,$data_insert);
              if($data)
              {
                echo '<script>window.location.href="../technology_update.php"</script>';
              }else
              {
                echo '<script>window.location.href="../technology_update.php?addedError=true"</script>';
              }
